...
import com.casadetasha.kexp.petals.accessor.*
import com.casadetasha.kexp.petals.annotations.BasePetalMigration
import com.casadetasha.kexp.petals.data.DataPetalData
import com.casadetasha.kexp.petals.data.OptionalParentPetalClassData
import com.casadetasha.kexp.petals.data.ParentPetalClassData
...
class NestedDataPetalTest: ContainerizedTestBase() {

    private val tableMigrations: Set<BasePetalMigration> = setOf(
        `TableMigrations$data_petal`(),
    ...
    @Test
    fun `nested accessor exports to data as ID`() {
        val nestedPetal = NestedPetalClass.create(name = "hola nestie")
        val petalData: ParentPetalClassData = ParentPetalClass.create(
            name = "My name",
            nestedPetal = nestedPetal
        ).asData()

        assertThat(petalData.name).isEqualTo("My name")
        assertThat(petalData.nestedPetalId).isEqualTo(nestedPetal.id)
    }

    @Test
    fun `null optional nested accessor exports to data as null ID`() {
        val petalData: OptionalParentPetalClassData = OptionalParentPetalClass.create(
            name = "My name",
            nestedPetal = null
        ).asData()

        assertThat(petalData.name).isEqualTo("My name")
        assertThat(petalData.nestedPetalId).isEqualTo(null)
    }
}
